...
from isaac_scraper.github_client import sanitize_log
from isaac_scraper.exceptions import AuthenticationError, RepositoryNotFoundError
...
class TestConfig:
    ...
    def test_loads_from_env(self, mock_token):
        config = Config()
        assert config.github_token.get_secret_value() == "ghp_test_12345"
...
class TestImages:
    ...
    def test_detects_image_formats(self, selector):
        assert selector.is_image("x.png")
        assert selector.is_image("X.SVG")
        assert not selector.is_image("x.md")
    
    def test_prefers_architecture_keyword(self, selector, mock_content_file):
        images = [
            mock_content_file("random.png", 5000),
            mock_content_file("architecture.png", 1000),
        ]
        assert selector.select_best(images).name == "architecture.png"
    
    def test_ignores_icons(self, selector, mock_content_file):
        images = [
            mock_content_file("icon.png"),
            mock_content_file("diagram.png"),
        ]
        assert selector.select_best(images).name == "diagram.png"
...
class TestScraper:
    ...
    @patch("isaac_scraper.github_client.Github")
    def test_connects_to_github(self, mock_github_class, mock_token):
        mock_repo = MagicMock()
        ...
        assert scraper._client.full_name == "test/repo"
    
    @patch("isaac_scraper.github_client.Github")
    def test_invalid_token_raises(self, mock_github_class, mock_token):
        from github import BadCredentialsException
        mock_github_class.return_value.get_repo.side_effect = BadCredentialsException(401, {}, {})
        
        with pytest.raises(AuthenticationError, match="Invalid GitHub token"):
            GitScraper(Config(github_token="bad", repo_name="a/b"))
    
    @patch("isaac_scraper.github_client.Github")
    def test_repo_not_found_raises(self, mock_github_class, mock_token):
        from github import UnknownObjectException
        mock_github_class.return_value.get_repo.side_effect = UnknownObjectException(404, {}, {})
        
        with pytest.raises(RepositoryNotFoundError, match="Repository not found"):
            GitScraper(Config(github_token="x", repo_name="no/exist"))
    
    @patch("isaac_scraper.github_client.Github")
    def test_crawl_processes_readme(self, mock_github_class, mock_token, tmp_path, mock_content_file):
        import base64
    ...
    @patch("isaac_scraper.github_client.Github")
    def test_respects_max_depth(self, mock_github_class, mock_token, tmp_path):
        mock_repo = MagicMock()
        mock_repo.full_name = "test/repo"
    ...
    @patch("isaac_scraper.github_client.Github")
    def test_saves_json_atomically(self, mock_github_class, mock_token, tmp_path):
        mock_repo = MagicMock()
        mock_repo.full_name = "test/repo"
        mock_repo.get_contents.return_value = []
        mock_github_class.return_value.get_repo.return_value = mock_repo
